...
4) Move the robot default configurations:
	Position 1: Robot far from the sensor: rosrun calibrate_rgb_sensor move_robot_back.py
	Position 2: Robot close to the sensor: rosrun calibrate_rgb_sensor test_move_init_joints_before_calib_rgb_sensor.py
...
roslaunch ur_modern_driver ur10_bringup.launch robot_ip:=192.168.1.3

>> moveit

roslaunch ur10_moveit_config ur10_moveit_planning_execution.launch 

>> initial_position----any_servo----before_calib

source ros/rgb_sensor/devel/setup.bash 

rosrun calibrate_rgb_sensor move_robot_back.py

before exp :::

rosrun calibrate_rgb_sensor test_move_init_joints_before_calib_rgb_sensor.py
...
source ros/rgb_sensor/devel/setup.bash 
roslaunch libuvc_camera webcam_start.launch

>> camera_image_processing

source ros/rgb_sensor/devel/setup.bash 
rosrun rgb_sensor rgb_sensor_node.py 

>> all_data 

source ~/ros/rgb_sensor/devel/setup.bash
rosrun calibrate_rgb_sensor subscribe_state_node

>> rosbag

source ~/ros/rgb_sensor/devel/setup.bash

create some folder, and then  

rosbag record /all_data_calib

>> RUN experiment

>> desired_velocity ---- moveit
...
rosrun calibrate_rgb_sensor broadcast_desired_pose_for_calib

>> firefox

from Downloads/firefox
